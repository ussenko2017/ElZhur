...
class RoomsIndexEndpoint(Endpoint, GetListMixin, CreateMixin):
    ...
    def get_response_handler_params(self, **params):
        params['serializer'] = rooms_serializer
        return params
...
class RoomsObjectEndpoint(Endpoint, GetObjectMixin,
    ...
    def get_object(self):
        ...
        obj = session.query(Room).filter(Room.id == obj_id).one_or_none()
        if not obj:
            payload = {
                "message": "Rooms object not found.",
            }
